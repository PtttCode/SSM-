...
import com.patir.service.DepartmentService;
...
@Controller
@RequestMapping(value="/patir/dept")
public class DepartmentController {
    ...
    public JsonMsg deleteDept(@PathVariable("deptId") Integer deptId){
        int res = 0;
        if (deptId > 0){
            res = departmentService.deleteByDeptId(deptId);
        }
        if (res != 1){
    ...
    public JsonMsg getTotalPages(){
        ...
        int limit = 5;
        //总记录数
        int totalItems = departmentService.getDeptCounts();
    ...
    public JsonMsg getDeptById(@PathVariable("deptId") Integer deptId){
        ...
        if (deptId > 0){
            department = departmentService.selectDeptById(deptId);
        }
        if (department != null){
    ...
    public ModelAndView getDeptList(@RequestParam(value = "pageNo", defaultValue = "1") Integer pageNo){
        ...
        List<Department> departments = departmentService.selectDeptByLO(offset, limit);
